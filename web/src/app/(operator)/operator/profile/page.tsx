...
import { User, Mail, Lock, Camera, Save, X, Edit2, Shield, Building2, Phone, CheckCircle } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { updateProfileAPI, getOperatorDetailsAPI } from '@/lib/api'
...
export default function OperatorProfilePage() {
  ...
  useEffect(() => {
    if (user) {
      ...
      // Fetch operator details if user has operatorId
      if (user.operatorId) {
        fetchOperatorDetails(user.operatorId)
      }
    }
  }, [user])
  ...
  const fetchOperatorDetails = async (operatorId: string) => {
    try {
      ...
      const operatorData = await getOperatorDetailsAPI(operatorId)
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Operator Profile</h1>
        <p className="text-gray-600 dark:text-gray-400 mt-2">Manage your operator account</p>
      </div>

      {/* Operator Badge Info */}
      <div className="bg-linear-to-r from-blue-600 to-blue-700 rounded-lg shadow-md p-6 text-white">
        <div className="flex items-center gap-4">
          <div className="bg-white/20 rounded-full p-3">
            <Shield className="w-8 h-8" />
          </div>
          <div>
            <h2 className="text-xl font-bold">Operator Account</h2>
            <p className="text-blue-100 mt-1">Manage your buses, routes, and trips</p>
          </div>
        </div>
      </div>
        ...
        {loadingOperator ? (
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex items-center gap-2 px-4 py-3 bg-gray-50 dark:bg-gray-700 rounded-md">
                <Building2 className="w-5 h-5 text-gray-400" />
                <span className="text-gray-900 dark:text-white font-medium">{operator.name}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="flex items-center gap-2 px-4 py-3 bg-gray-50 dark:bg-gray-700 rounded-md">
                  ...
                  <span className="text-gray-900 dark:text-white">{operator.contactEmail}</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 px-4 py-3 bg-gray-50 dark:bg-gray-700 rounded-md">
                  ...
                  <span className="text-gray-900 dark:text-white">{operator.contactPhone || 'Not set'}</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <Badge
                  variant={
                    operator.status === 'approved' ? 'success' : operator.status === 'pending' ? 'warning' : 'default'
                  }
                ...
                >
                  {operator.status === 'approved' && <CheckCircle className="w-4 h-4 mr-2" />}
                  {operator.status.toUpperCase()}
                </Badge>
                {operator.status === 'approved' && operator.approvedAt && (
                  <span className="text-sm text-gray-600 dark:text-gray-400">
                    Approved on{' '}
                    {new Date(operator.approvedAt).toLocaleDateString('en-US', {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric',
                    })}
          ...
          <div className="text-center py-8 text-gray-500 dark:text-gray-400">No operator information available</div>
        )}
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">JPG, JPEG or PNG. Max size 10MB.</p>
            {avatarFile && (
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">Primary contact for system notifications</p>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2 px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800">
              ...
              {isEditing ? (
                ...
                <span className="text-gray-900 dark:text-white">{user.displayName || 'Not set'}</span>
              )}
            </div>
            <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
              This name will be displayed across the operator panel
            </p>
          </div>
        ...
        {isChangingPassword ? (
          <div className="space-y-4">
            ...
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-md p-4">
              <p className="text-sm text-yellow-800 dark:text-yellow-200">
                <strong>Security Note:</strong> Changing your password will log you out of all other devices for
                security reasons.
              </p>
            </div>
          ...
          <div>
            <p className="text-gray-600 dark:text-gray-400 mb-4">
              Keep your operator account secure by using a strong, unique password
            </p>
