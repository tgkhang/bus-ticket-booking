    ...
    <div className="bg-white rounded-lg shadow-md hover:shadow-xl transition-all duration-300 border border-gray-200 overflow-hidden group">
      <div className="p-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-6">
              ...
              <div className="flex-shrink-0 text-right">
                ...
                <div className="text-sm text-gray-600 mt-1 flex items-center gap-1 justify-end">
                  <MapPin className="w-3 h-3" />
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 lg:gap-6">
            ...
            <Link href={`/booking/seats/${trip.id}`} className="w-full sm:w-auto">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {activeAmenities.slice(0, 4).map((amenity) => {
                  ...
                  return (
                    <div
                      key={amenity}
                      className="flex items-center gap-1 bg-gray-100 px-3 py-1.5 rounded-full text-xs text-gray-700"
                      title={config.label}
                    >
                ...
                {activeAmenities.length > 4 && (
                  <div className="flex items-center gap-1 bg-gray-100 px-3 py-1.5 rounded-full text-xs text-gray-700">
      ...
      {trip.status !== 'scheduled' && (
        <div className="bg-yellow-50 border-t border-yellow-200 px-6 py-2">
          <p className="text-xs text-yellow-800 capitalize">{trip.status}</p>
        </div>
      )}
    </div>
  )
